...
const ActivityHeatmap = ({ heatmap, monthLabels }) => {
  const [hoveredCell, setHoveredCell] = useState(null);
  

  const createHeatmapGrid = () => {
    const grid = Array(26).fill().map(() => Array(7).fill(0));
    const rawCounts = Array(26).fill().map(() => Array(7).fill(0));
    ...
    heatmap.forEach(dayData => {
      try {
        const [year, month, day] = dayData.date.split('-').map(num => parseInt(num, 10));
        const adjustedYear = year > today.getFullYear() ? year - 1 : year;
        ...
        if (diffDays >= 0 && diffDays < 182) {
          ...
          if (weekIndex < 26) {
            rawCounts[weekIndex][dayIndex] = dayData.value;
          }
        }
      } catch (error) {
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm">
      ...
      <div className="w-full overflow-hidden relative">
              ...
              <div key={weekIndex} className="flex flex-col gap-1">
                {week.map((day, dayIndex) => (
                  <div
                    ...
                    title={`${formatDate(weekIndex, dayIndex)}: ${rawCounts[weekIndex][dayIndex]} submissions`}
        ...
        {hoveredCell && (
          <div className="absolute bg-white p-2 rounded-md shadow-lg border border-gray-200 z-10 text-xs"
               style={{ 
            ...
            <div className="mt-1 text-gray-600">
              {rawCounts[hoveredCell.weekIndex][hoveredCell.dayIndex] || 0} submissions
            </div>
            {rawCounts[hoveredCell.weekIndex][hoveredCell.dayIndex] > 0 && (
              <div className="mt-1 text-green-600 font-medium">
                Activity level: {grid[hoveredCell.weekIndex][hoveredCell.dayIndex]}/4
              </div>
            )}
          </div>
        )}
