...
const LeetCode = () => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        const transformedData = {
          ...
          recentSubmissionList: (data.recentSubmissions || []).map(submission => ({
            ...submission,
            question: {
  ...
  const processRecentSubmissions = () => {
    ...
    return userData.recentSubmissionList
      ...
      .map(submission => ({
        ...
        tags: submission.question?.topicTags?.map(tag => tag.name) || ['Algorithm']
      }));
  };
    ...
    return (
      <div className="p-8">
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
          ...
          <p className="mt-2">Please check if the LeetCode API is available or try again later.</p>
        </div>
      </div>
    );
  }
  ...
  const stats = [
    ...
    {
      title: 'Max Streak',
      value: <MaxStreak submissionCalendar={profileCalendar} isCompact={true} />,
